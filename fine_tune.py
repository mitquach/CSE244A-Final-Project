...
def load_startpoint(checkpoint_path, model, device='cpu'):
    print(f"Loading starting values for training from {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location=device)
    model.load_state_dict(checkpoint["model_dict"])
...
if not "augment_mode" in training_config:
    ...
    elif training_config["augment_mode"] == "3augment":
        # The augmentation strategy listed in the Deit paper
        augmented_transform = new_data_aug_generator(AA3)
    else:
...
# The unlabeled dataset is notably larger
...
elif "start_name" in training_config:
    start_model_path = os.path.join(model_prefix, training_config["start_name"])
    start_checkpoint_path = os.path.join(start_model_path, "checkpoint-best.pth")
    load_startpoint(start_checkpoint_path, model, device=device)
...
for epoch in range(start_epoch, num_epochs):
    ...
    if ("unlabeled_start_epoch" in training_config and epoch >= training_config["unlabeled_start_epoch"] ):
