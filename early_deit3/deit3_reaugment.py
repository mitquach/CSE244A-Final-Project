...
# the idea on this one is basically that we augment our data once, then run training on that repeatedly.
# what if we re-load the dataset with different augmentation each epoch, since the augmentation strategy
# here involves a random component?  That might do a better job of inflating the data space.
...
training_config = load_config("freeze10_explr_1en4_0.8_unlabeled_3AA")
...
val_dataset = ImageDataset(os.path.join(data_prefix,'train/labeled'), label_csv=os.path.join(data_prefix,'train_labeled.csv'), transform=transform)
unlabeled_dataset = ImageDataset(os.path.join(data_prefix,'train/unlabeled'), transform=new_data_aug_generator(AA3))
...
for epoch in range(start_epoch, num_epochs):
    ...
    # Start using unlabeled data after a couple epochs have passed
    if (epoch > 3):
        for image_arr in tqdm.tqdm(unlabeled_data, desc=f"Train (unlabeled) ({epoch+1}/{num_epochs})"):
            images = image_arr[0]
            images = images.to(device)
            # first we predict labels for the unlabeled data.  This is done in eval mode because otherwise
            # I'm not sure how to avoid possible biases from fancy stuff like dropout, etc.
            model.eval()
            with torch.no_grad():
                synthetic_labels = model(images)
                labels = torch.max(synthetic_labels,1).indices
                #labels = synthetic_labels.max(1)
            # Now that we have putative labels to train on, do a training pass using them
            model.train()
