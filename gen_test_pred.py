...
training_config = load_config(args.settingsFile)
...
val_dataset = ImageDataset(os.path.join(data_prefix,'train/labeled'), label_csv=os.path.join(data_prefix,'train_labeled.csv'), transform=transform)
unlabeled_dataset = ImageDataset(os.path.join(data_prefix,'train/unlabeled'), transform=new_data_aug_generator(AA3), return_filenames = True)
...
elif "start_point" in training_config:
    start_model_path = os.path.join(model_prefix, training_config["model_name"])
    start_checkpoint_path = os.path.join(model_path, "checkpoint-best.pth")
    load_checkpoint(checkpoint_path, model, optimizer, lr_scheduler, device=device)
...
if True:
    train_dataset_for_compare = ImageDataset(os.path.join(data_prefix,'train/labeled'), transform=transform, return_filenames=True)
    all_train_data = DataLoader(train_dataset_for_compare, batch_size=batch_size, shuffle=True, num_workers=8, persistent_workers=True)
    results = []
    with torch.no_grad():
        for images, filenames in tqdm.tqdm(all_train_data, desc="Generating training labels for comparison"):
            images = images.to(device)
            outputs = model(images)
            _, predicted = outputs.max(1)  # Get the predicted class IDs
    ...
    create_csv_with_number("training_results", results)
    
# supplemental stuff added later to generate a label set and display which validation
# images seem to be failing

if False:
    with torch.no_grad():
        for images, filenames in tqdm.tqdm(unlabeled_data, desc="Unlabeled training data labeling"):
            #images = image_arr[0]
            images = images.to(device)
            outputs = model(images)
            _, predicted = outputs.max(1)  # Get the predicted class IDs

            print(f"{len(images)} images -> {len(predicted.cpu().numpy())} labels")
            # Store filename and predicted label
            results.extend(zip(filenames, predicted.cpu().numpy()))

    print(f"Wound up with {len(results)} labeled images")
    create_csv_with_number("unlabeled_training_labels", results)
    results = []

if False:
    bad_results = []
    with torch.no_grad():
        for cpuimages, labels in tqdm.tqdm(labeled_val_data, desc="Validation failure assessment"):
            images = cpuimages.to(device)
            outputs = model(images)
            _, predicted = outputs.max(1)  # Get the predicted class IDs

            # Store filename and predicted label
            cpuPred = predicted.cpu()
            for i in range(len(images)):
                if len(bad_results) < 8 and labels[i] != cpuPred[i]:
                    bad_results.extend(zip(cpuimages[i], f"Pred: {predicted[i]}, Real: {labels[i]}"))

    for idx in range(min(8, len(bad_results))):
        plt.subplot(2, 4, idx + 1)
        image, label = bad_results[idx]
        img = image
    #    img = image.permute(1, 2, 0) # Convert from Tensor format
    #    img = img/2 + 0.5 # This roughly un-normalizes them back to a valid range for imshow - Daniel
        plt.imshow(img)
        plt.title(label)
        plt.axis('off')
